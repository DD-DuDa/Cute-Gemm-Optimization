...
double TestDecodingKernelPerformance(int seqlen_kv, int repeat) {
    ...
    torch::Tensor K_host = torch::ones({bs, seqlen_kv, num_heads, head_dim}, torch::dtype(torch::kHalf));
    torch::Tensor V_host = torch::ones({bs, seqlen_kv, num_heads, head_dim}, torch::dtype(torch::kHalf));
    ...
    at::Tensor seqlens_k;
    std::optional<const at::Tensor> opt_K_new_device = std::nullopt;
    std::optional<const at::Tensor> opt_V_new_device = std::nullopt;
    std::optional<const at::Tensor> opt_seqlens_k = std::make_optional(seqlens_k);
    
    // Warm up
    for (int i = 0; i < 5; ++i)
        mha_fwd_kvcache(Q_device, K_device, V_device, opt_K_new_device, opt_V_new_device, opt_seqlens_k);
