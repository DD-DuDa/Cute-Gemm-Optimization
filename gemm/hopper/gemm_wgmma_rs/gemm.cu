...
gemm_device(const T *Aptr, const T *Bptr, T *Dptr, int m, int n, int k) {
    ...
    Tensor tAsA = s2r_thr_copy_a.partition_S(sA);
    ...
    for (int itile = 0; itile < ntile; ++itile) {
        ...
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, _), tCrA_view(_, _, _));
...
void gemm_wgmma(T *a, T *b, T *c, int M, int N, int K) {
    ...
    using TiledMMA = decltype(make_tiled_mma(cute::GMMA::rs_op_selector<T, T, T, TileShape_MNK>()));
    ...
    void const* kernel_ptr = reinterpret_cast<void const*>(
        &gemm_device<T, BM, BN, BK, TiledMMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, S2RCopyAtomA>
    );
